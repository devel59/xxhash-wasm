...
import { xxhash as xxhashSync } from "../src/node";
...
test("sync hasher produces same hash as async one", async (t) => {
  const input = "abcd";
  const hasherAsync = await xxhash();
  const hasherSync = xxhashSync();

  t.is(hasherAsync.h32(input), hasherSync.h32(input));
  t.is(hasherAsync.h64(input), hasherSync.h64(input));
});
